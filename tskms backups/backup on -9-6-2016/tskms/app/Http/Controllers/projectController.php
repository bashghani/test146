...
$to_date=$to_date->toDateString();
	...
	$input['start_date']=$from_date;
	   $input['expected_comp_date']=$to_date;
	...
	public function ser_proj()
{
  ...
  $pending_proj_countp=project::where('status', '=', 'pending' )->where
 ('expected_comp_date', '<',  $today)->where('proj_assign_by_id', '=',  $tl_id)->count();
		   return view('pmhome44',array('matched_projects'=>$match_projs,'matched_tls'=>$tls_for_proj,'pending_proj_count'=>$pending_proj_countp));   
...
$to_date	= Carbon::now();
	$from_date->year($segr_fromdate[0])->month($segr_fromdate[1])->day($segr_fromdate[2])->hour(0)->minute(0)->second(0)
	->toDateTimeString();
	//echo $from_date->toDateString();echo "yepi"; die;
	$from_date=$from_date->toDateString();
	//echo $from_date;echo "yes it is";die;
$to_date->year($segr_tomdate[0])->month($segr_tomdate[1])->day($segr_tomdate[2])->hour(0)->minute(0)->second(0)->toDateTimeString();
$to_date=$to_date->toDateString();
//echo $to_date;
//$from_date=$from_date->toDateString();
	$input['start_date']=$from_date;
	   $input['expected_comp_date']=$to_date;
		...
		$proj =project::find($input['proj_id']);
		 ...
		 $proj->assigned_to = $segr_forproj[0];
		 $proj->proj_assign_to_id=$segr_forproj[1];
	     $proj->start_date = $input['start_date'];
		 $proj->expected_comp_date = $input['expected_comp_date'];
		 
		 
		  $proj->proj_assign_by =Auth::user()->first_name." ".Auth::user()->last_name;
		   $proj->proj_assign_by_id =Auth::user()->id;
  ...
  $pending_proj_countp=project::where('status', '=', 'pending' )->where
 ('expected_comp_date', '<',  $today)->where('proj_assign_by_id', '=',  $tl_id)->count();
		   return view('pmhome44',array('matched_projects'=>$match_project,'matched_tls'=>$tls_for_proj,'pending_proj_count'=>$pending_proj_countp));  
...
public function allprojects()
    {//echo "on detail points sys";
	$proj_mgr_id=Auth::user()->id;
	
	 $match_project=project::where('proj_assign_by_id', '=', $proj_mgr_id)->get();
		  $tls_for_proj=user::where('type_of', '=', "team_lead" )->get();
		  $tl_id=Auth::user()->id;
    $today = Carbon::today();
  $pending_proj_countp=project::where('status', '=', 'pending' )->where
...
public function cancelled_projects()
	...
	$proj_mgr_id=Auth::user()->id;
	 ...
	 $match_project=project::where('proj_assign_by_id', '=', $proj_mgr_id)->where('status', '=', 'cancelled')->get();
		  $tls_for_proj=user::where('type_of', '=', "team_lead" )->get();
		  $tl_id=Auth::user()->id;
    $today = Carbon::today();
  $pending_proj_countp=project::where('status', '=', 'pending' )->where
...
public function completed_projects()
	...
	$proj_mgr_id=Auth::user()->id;
	 ...
	 $match_project=project::where('proj_assign_by_id', '=', $proj_mgr_id)->where('status', '=', 'complete')->get();
		  $tls_for_proj=user::where('type_of', '=', "team_lead" )->get();
		  $tl_id=Auth::user()->id;
    $today = Carbon::today();
  $pending_proj_countp=project::where('status', '=', 'pending' )->where
...
public function active_projects()
	...
	$proj_mgr_id=Auth::user()->id;
	 ...
	 $match_project=project::where('proj_assign_by_id', '=', $proj_mgr_id)->where('status', '=', 'pending')->get();
		  $tls_for_proj=user::where('type_of', '=', "team_lead" )->get();
		  $tl_id=Auth::user()->id;
    $today = Carbon::today();
  $pending_proj_countp=project::where('status', '=', 'pending' )->where
...
public function overdue_projects_show()
    {//echo "on detail points sys";
	$tl_id=Auth::user()->id;
  $today = Carbon::today();
	$proj_mgr_id=Auth::user()->id;
	
	 $match_project=project::where('status', '=', 'pending' )->where
 ('expected_comp_date', '<',  $today)->where('proj_assign_by_id', '=',  $tl_id)->get();
		  $tls_for_proj=user::where('type_of', '=', "team_lead" )->get();
