...
@endif
                @if($flag_stat->flag_status==1)      
                
                </br></br></br>
                <div  align="center"> 
    <div id="tmdet" align="center"> 
         {{--   <h4> {{$flag_task_details['name']}}	</h4>
         <h6>  --}}
        
        <h4> Task Name &nbsp;&nbsp;&nbsp;{{$flag_task_details['task_name']}}
          </h4>
        Task Id  {{$flag_task_details['task_id']}}
        {{--   <br>Email  {{$flag_task_details['email']}}
          <br>Designation  {{$ser_user1['type_of']}} &nbsp;Team member
           <br>Deparatment &nbsp;  {{$flag_task_details['dept']}} --}}
        
         @endif	
         <div class="clock" style="margin:2em;"></div>
	<div class="message"></div>
   @if($flag_stat->flag_status==1)
  {{-- <button id="start"  class="btn btn-info">
                                    <i class="fa fa-btn fa-user"></i>Start Timer
                                </button>
     <button id="stop"  class="btn btn-info">
                                    <i class="fa fa-btn fa-user"></i>Stop Timer
                             
                                </button>--}}
                                <form class="form-horizontal" role="form" method="POST" action="{{ url('/stop_timer_task') }}"  style="display:inline;">
                        {!! csrf_field() !!}
                      <input type="hidden" name="flag_task_id"  value="{{$flag_task_details['id']}}">
                        <input type="hidden" name="flag_user_id"  value="{{ $flag_task_details['usr_id']}}">
                         <input type="hidden" name="task_id"  value="{{ $flag_task_details['task_id']}}">
                       <input type="hidden" name="flag_task_name"  value="{{$flag_task_details['task_name']}}">
                        <input type="hidden" name="flag_task_start_time"  value="{{$flag_task_details['flag_start_time']}}">
                     {{--   <input type="hidden" name="to_tl_name"  value="{{$task_tl_name}}">--}}
                                <button type="submit"  class="btn btn-info">
                                    <i class="fa fa-clock-o"></i>Stop Timer
                                </button></form>
	{{--<button id="stop">Start Clock</button>--}}
  {{--  <button id="stop">Stop Clock</button> --}}
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			clock = $('.clock').FlipClock({{$differ_in_sec}}, {
...
<script src="http://getbootstrap.com/dist/js/bootstrap.min.js"></script>
 ...
 {{--   <td>{{$ser_proj['task_assigned_to']}}</td> --}}
   ...
   {{-- <td>{{$ser_proj['task_total_time']}}</td> --}}
      <td> 
     ...
     <td><form class="form-horizontal" role="form" method="POST" action="{{ url('/start_timer_task') }}"  style="display:inline;">
                        {!! csrf_field() !!}
                      <input type="hidden" name="flag_task_id"  value="{{ $ser_proj['id']}}">
                        <input type="hidden" name="flag_task_name"  value="{{ $ser_proj['task_name']}}">
                         <input type="hidden" name="flag_usr_id"  value="{{ $ser_proj['assgn_to_id']}}">
                     {{--   <input type="hidden" name="to_proj_name"  value="{{$task_project_name}}">
                        <input type="hidden" name="to_tl_id"  value="{{$task_tl_id}}">
                        <input type="hidden" name="to_tl_name"  value="{{$task_tl_name}}">--}}
                                <button type="submit"  class="btn btn-info">
                                    <i class="fa fa-btn fa-clock-o"></i>Start Timer
                                </button></form> </td>
