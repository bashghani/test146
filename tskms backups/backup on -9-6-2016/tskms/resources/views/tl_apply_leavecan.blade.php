    ...
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                <div class="panel-heading">Team lead Leave apply section</div>
                 <div> </div>
                 <div id="topcorner" > 
                                 <a href="{{ url('/applied_all_leave_app_tl') }}"><button  class="btn btn-info">
                                    <i class="fa fa-calendar-plus-o"></i> &nbsp;All Applications
                                </button></a> 
                                
                                <a href="{{ url('/applied_cancelled_leave_app_tl') }}"><button  class="btn btn-info">
                                    <i class="fa fa-btn fa-calendar-times-o"></i>Cancelled Applications
                                </button></a>  
                                 
                                <a href="{{ url('/applied_approved_leave_app_tl') }}"><button  class="btn btn-info">
                                    <i class="fa fa-btn fa-calendar-check-o"></i>Approved Applications
                                </button></a> 
                                                                
                                
                                <a href="{{ url('/applied_pending_leave_app_tl') }}"><button  class="btn btn-info">
                                    <i class="fa fa-btn fa-calendar-o"></i>Pending Applications
                                </button></a> </a> </div>
                <div class="panel-body">
                 <h1>Team lead Leave apply section</h1></br></br>
...
<select name="team_lead_for_approval" required>
  <option disabled selected > -- Person for approval-- </option>
 @foreach($matched_tls as $teamleaders)
   <option value="{{$teamleaders->first_name.'-'.$teamleaders->id}}">
   {{$teamleaders->first_name}}---- ID:{{$teamleaders->id}}</option>
   @endforeach
  </select>
...
<script src="http://getbootstrap.com/dist/js/bootstrap.min.js"></script>
    ...
    <td>{{$detail_leaves['approval_from_tl']}}</td>
    <td>{{$detail_leaves['tl_comment']}}</td>
    <td>{{$detail_leaves['status']}}</td>
     <td>{{$detail_leaves['Status_updated_by']}}</td>
...
<tr>
 ...
 <div class="panel-body">
 <form class="form-horizontal" role="form" method="POST" action="{{ url('/tm_edit_leave') }}">
                        {!! csrf_field() !!}
...
<select name="leave_type" required>
 ...
 <option selected value="{{$detail_leaves['applicant_comment']}}">{{$detail_leaves['leave_type']}} leave</option>
  <option value="casual">Casual leave</option>
  <option value="sick">Sick leave</option>
  <option value="annual">Annual leave</option>
   <option value="other">Other</option>
...
@if ($errors->has('leave_type'))
                        ...
                        <div class="form-group{{ $errors->has('leave_type') ? ' has-error' : '' }}">
                            <label class="col-md-4 control-label">leave_type</label>
...
@if ($errors->has('status'))
                         ...
                         <div class="form-group{{ $errors->has('Status_updated_by') ? ' has-error' : '' }}">
                            <label class="col-md-4 control-label">{{$detail_leaves['status']}} by</label>

                            <div class="col-md-6">
                                <input type="text" class="form-control" name="Status_updated_by" 
                                value="{{$detail_leaves['Status_updated_by']}}" disabled>

                                @if ($errors->has('Status_updated_by'))
                                    <span class="help-block">
                                        <strong>{{ $errors->first('Status_updated_by') }}</strong>
                                    </span>
                                @endif
                            </div>
                        </div>
                         <div class="form-group{{ $errors->has('tl_comment') ? ' has-error' : '' }}">
                            <label class="col-md-4 control-label">Teamlead comment</label>
  ...
  <input type="text"  id="{{$disp=$detail_leaves['id']."abcef"}}" class="form-control" name="start_date" 
   value="{{$detail_leaves['start_date']}}" disabled >
                        ...
                        <div class="form-group{{ $errors->has('end_date') ? ' has-error' : '' }}">
                           ...
                           {{-- <input type="text" class="form-control" name="applicant_comment" 
                        value="{{ old('end_date') }}" required> --}}
  <input type="text"  id="{{$disp=$detail_leaves['id']."abcefd"}}" class="form-control" name="end_date" value="{{$detail_leaves['end_date']}}" disabled >
...
@if ($errors->has('team_lead_for_approval'))
                         ...
                         <div class="form-group{{ $errors->has('approval_from_tl') ? ' has-error' : '' }}">
                            <label class="col-md-4 control-label">Team lead</label>

                            <div class="col-md-6">
                                <input type="text" class="form-control" name="approval_from_tl" 
                                value="{{$detail_leaves['approval_from_tl']}}" disabled>

                                @if ($errors->has('approval_from_tl'))
                                    <span class="help-block">
                                        <strong>{{ $errors->first('approval_from_tl') }}</strong>
                                    </span>
                                @endif
                            </div>
                        </div>
