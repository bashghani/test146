...
<div class="container">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">Dashboard For project manager</div><div id="topcorner" > <a href="{{ url('/pointsys') }}"><button  class="btn btn-primary">
                                ...
                                <a href="{{ url('/make_team') }}"><button  class="btn btn-primary">
                                    <i class="fa fa-btn fa-user"></i>Make team
                                </button>
                                &nbsp;
	...
	function show(select_item) {
	   @foreach($tl_projects as $project_member)
		...
		newd="{{$project_member->id}}";
		
	    if (select_item == {{$project_member->id}}) {
		    document.getElementById(newd).style.visibility='visible';
			document.getElementById(newd).style.display='block';
			Form.fileURL.focus();
		} 
		
		else{
			document.getElementById(newd).style.visibility='hidden';
...
<label>Fruit:</label>
    <select name="Fruit" onchange="java_script_:show(this.options[this.selectedIndex].value)">
        <option value='pear'>perynow</option>
                    ...
                    <form class="form-horizontal" role="form" method="POST" action="{{ url('/maketask') }}">
                        ...
                        <div class="form-group{{ $errors->has('task_name') ? ' has-error' : '' }}">
                          ...
                          <div class="form-group{{ $errors->has('task_assigned_to') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-2">
               @foreach($teams_by as $all_team_by)             
    <div id="{{$all_team_by['team_project_id']}}">                         
<select name="task_assigned_to">
  ...
  <option disabled selected > -- select team member -- </option>
   ...
   @if($all_team_by['id']==$team_member['mem_team_id'])
   <option value="{{$all_team_by['id']}}">
   {{$team_member->member_name}}---- ID:{{$team_member->member_id}}--{{$all_team_by['id']}}--{{$all_team_by['team_project_id']}}
...
@if ($errors->has('task_assigned_to'))
                       ...
                       <div class="form-group{{ $errors->has('task_project_name') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-2">
<select name="task_project_name" onchange="java_script_:show(this.options[this.selectedIndex].value)" >
  <option disabled selected > -- select project -- </option>
 @foreach($tl_projects as $project_member)
   <option value="{{$project_member->project_name}}">{{$project_member->project_name}}--{{$project_member->id}}</option>
   @endforeach
  </select>
