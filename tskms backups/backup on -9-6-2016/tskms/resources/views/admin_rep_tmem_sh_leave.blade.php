...
<form class="form-horizontal" role="form" method="POST" action="{{ url('/admin_sh_leave_datefilter') }}">
                        {!! csrf_field() !!}
...
<script src="http://getbootstrap.com/dist/js/bootstrap.min.js"></script>
	...
	$etime= Carbon::now();
	$stime= Carbon::now();
	$etime=Carbon::parse($ser_user['end_time']);
	$stime= Carbon::parse($ser_user['start_time']);
	$diff= $etime->diffInMinutes($stime)/60; 
	$total_no_hours= $total_no_hours+$diff;
...
//	echo $ser_user['start_date']->toDateString();;
    ...
    <td>{{$ser_user['applicant_id']}}</td>
     <td>{{$ser_user['applicant_name']}}</td>
    <td>{{$ser_user['leave_type']}}</td>
    <td>{{$ser_user['start_time']}}</td>
    <td>{{$ser_user['end_time']}}</td>
...
?>
  ...
  @endforeach
    ...
    <tr>
                    ...
                    <th>Total points </th>
                     <th colspan="4">Total No. of Hours</th>
                   {{--   <th >Comment</th>
                    <th>Deptarment</th>
                     <th>Day Points</th>--}}
                    ...
                    {{-- <td>Deptarment</td>
                     ...
                     <td colspan="4">{{$total_no_hours}}</td>
                    {{--  <td>Edit</td> --}}
